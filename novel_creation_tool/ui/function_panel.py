...
try:
    from ui.scene_segmentation_window import SceneSegmentationWindow
except ImportError:
    # 如果场景分割窗口不可用，给出错误提示
    SceneSegmentationWindow = None

try:
    from ui.shot_split_window import ShotSplitWindow
except ImportError:
    # 如果分镜头窗口不可用，给出错误提示
    ShotSplitWindow = None

try:
    from ui.keyframe_split_window import KeyframeSplitWindow
except ImportError:
    # 如果关键帧分割窗口不可用，给出错误提示
    KeyframeSplitWindow = None

try:
    from ui.keyframe_image_generation_window import KeyframeImageGenerationWindow
except ImportError:
    # 如果关键帧生图窗口不可用，给出错误提示
    KeyframeImageGenerationWindow = None
...
class FunctionPanelWidget(ttk.Frame):
    ...
    def show_scene_split(self):
        ...
        if SceneSegmentationWindow is None:
            messagebox.showerror("错误", "场景分割窗口模块未找到！")
            return
            
        # 创建场景分割窗口
        app = SceneSegmentationWindow(self.current_project_path)
    ...
    def show_shot_split(self):
        ...
        if ShotSplitWindow is None:
            messagebox.showerror("错误", "分镜头窗口模块未找到！")
            return
            
        # 创建分镜头窗口
        app = ShotSplitWindow(self.current_project_path)
        app.root.mainloop()
    ...
    def show_keyframe_split(self):
        ...
        if KeyframeSplitWindow is None:
            messagebox.showerror("错误", "关键帧分割窗口模块未找到！")
            return
            
        # 创建关键帧分割窗口
        app = KeyframeSplitWindow(self.current_project_path)
        app.root.mainloop()
    ...
    def show_keyframe_image_generation(self):
        ...
        if KeyframeImageGenerationWindow is None:
            messagebox.showerror("错误", "关键帧生图窗口模块未找到！")
            return
            
        # 创建关键帧生图窗口
        app = KeyframeImageGenerationWindow(self.current_project_path)
        app.root.mainloop()
